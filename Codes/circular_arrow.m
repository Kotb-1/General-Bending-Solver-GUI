function [all,f] = circular_arrow(axes1, radius, centre, arrow_angle, angle, direction, colour, head_size, head_style)
...
while i < 3
    ...
    pl(i) = plot(axes1,v{i}(1,:)+xc,v{i}(2,:)+yc,'Color', colour); % Plot arc, centered at P0
...
while i < abs(direction) + 1
    h(i) =annotation('arrow'); % arrow head
    set(h(i),'parent', axes1, 'position', position{i}, ...
